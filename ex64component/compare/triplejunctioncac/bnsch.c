...
ijloop{
     ...
     ijloop{
      ...
      EnEM = EnEM + h*h*( 
         ...
         + 0.5*SS/Cahn*( pow(nc[i][j]-oc[i][j],2)  
         + pow(nc2[i][j]-oc2[i][j],2) 
         + pow(nc3[i][j]-oc3[i][j],2) 
         + pow(1.0-nc[i][j]-nc2[i][j]-nc3[i][j] - (1.0-oc[i][j]-oc2[i][j]-oc3[i][j]),2)   )  );
...
double Ef(double **c, double **c2, double **c3,double **c4, int nxt, int nyt)
{
   ...
   ijloopt{ r = r + h*h*( 0.25*pow(c[i][j],2)*pow(c[i][j]-1.0,2) 
   + 0.25*pow(c2[i][j],2)*pow(c2[i][j]-1.0,2) 
         + 0.25*pow(c3[i][j],2)*pow(c3[i][j]-1.0,2)
         +0.25*pow(c4[i][j],2)*pow(c4[i][j]-1.0,2)  );  }
