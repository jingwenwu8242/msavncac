...
Hseudo-boundary condition:x:periodic ,y:Neunman
************************************************/
...
double pi, xleft, xright, yleft, yright, h, dt, gam, Cahn,  Ev, oEv, acount, int_r, or, r, nr, EnEO, EnEM,M,
        **worku, **workv, **workp, **mc, SS, CC,**ct, **sc, **smu, **mu, **P1, **P2, **P3,**P4 ,**alp, **intc1, **intc2,
        **intc3,**intc4, **c4, **oc4,**ic, **ic2, **ic3,**ic4;
char bufferc[1000]={0}, bufferc2[1000]={0}, bufferc3[1000]={0},bufferc4[1000]={0};
int main()
{
    /* extern int nx, ny, n_level,  c_relax, count, Num;
    extern double pi, xleft, xright, yleft, yright, h, dt, gam, Cahn, Ev,  oEv, acount, int_r, or, r, nr, EnEO, EnEM,
    ...
    double **c, **nc, **oc, **c2, **nc2, **oc2,**c3, **nc3, **oc3,**ic, **ic2, **ic3, **ic4,elapsed;
    clock_t start,end;
    
    FILE  *fc, *fc2, *fc3,*fc4, *myr, *myq,*mycpu;
    ...
    P1 = dmatrix(0, nx+1, 0, ny+1);
    P2 = dmatrix(0, nx+1, 0, ny+1);
    P3 = dmatrix(0, nx+1, 0, ny+1);
    P4 = dmatrix(0, nx+1, 0, ny+1);
   ...
   /*  sprintf(bufferc,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/datac.m");
    sprintf(bufferc2,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/datac2.m");
    sprintf(bufferc3,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/datac3.m");
    sprintf(bufferc4,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/datac4.m");
    ...
    augmenc(c4, nx, ny);
     ...
     ijloop{
      ...
      EnEO = EnEO + h*h*( 0.25*pow(c[i][j],2)*pow(c[i][j]-1.0,2) + 0.25*pow(c2[i][j],2)*pow(c2[i][j]-1.0,2) 
         + 0.25*pow(c3[i][j],2)*pow(c3[i][j]-1.0,2) +0.25*pow(c4[i][j],2)*pow(c4[i][j]-1.0,2)
         + 0.5*Cahn*( pow(c[i+1][j]-c[i][j],2)/(h*h) + pow(c[i][j+1]-c[i][j],2)/(h*h) )
         + 0.5*Cahn*( pow(c2[i+1][j]-c2[i][j],2)/(h*h) + pow(c2[i][j+1]-c2[i][j],2)/(h*h) )
         + 0.5*Cahn*( pow(c3[i+1][j]-c3[i][j],2)/(h*h) + pow(c3[i][j+1]-c3[i][j],2)/(h*h) )  
         + 0.5*Cahn*( pow(c4[i+1][j]-c4[i][j],2)/(h*h) + pow(c4[i][j+1]-c4[i][j],2)/(h*h) )  );
         
     }
       
    /* myr = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/Mene.m","w");
    myq = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctionch/dataout/Oene.m","w");
    
    fprintf(myr,"%16.12f \n",EnEO);
    fprintf(myq,"%16.12f \n",EnEO);
 */

   mycpu = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/cpu/dataoutput/cpuch.m","w");
  ...
  for( it = 1; it <= 1; it++){
    ...
    ijloop{
      ...
      P1[i][j] = c[i][j]*(c[i][j]-0.5)*(c[i][j]-1.0)/Ev;
      P2[i][j] = c2[i][j]*(c2[i][j]-0.5)*(c2[i][j]-1.0)/Ev;
      P3[i][j] = c3[i][j]*(c3[i][j]-0.5)*(c3[i][j]-1.0)/Ev;
      P4[i][j] = c4[i][j]*(c4[i][j]-0.5)*(c4[i][j]-1.0)/Ev;

      alp[i][j] = -(1.0/Num)*(P1[i][j] + P2[i][j] + P3[i][j] + P4[i][j] );
    ...
    ijloop{
        ...
        acount = acount + h*h*( P1[i][j]*r*(nc[i][j]-c[i][j])/dt
               + P2[i][j]*r*(nc2[i][j]-c2[i][j])/dt+P3[i][j]*r*(nc3[i][j]-c3[i][j])
               + P4[i][j]*r*( (1.0-nc[i][j]-nc2[i][j]-nc3[i][j])-(1.0-c[i][j]-c2[i][j]-c3[i][j]) )/dt );
    ...
    ijloop{
       ...
       intc4[i][j] = 1.0 - intc1[i][j] - intc2[i][j]- intc3[i][j];

      }
      


       Ev = Ef(intc1, intc2, intc3,intc4, nx, ny);
 

    ijloop{

      P1[i][j] = intc1[i][j]*(intc1[i][j]-0.5)*(intc1[i][j]-1.0)/Ev;
      P2[i][j] = intc2[i][j]*(intc2[i][j]-0.5)*(intc2[i][j]-1.0)/Ev;
      P3[i][j] = intc3[i][j]*(intc3[i][j]-0.5)*(intc3[i][j]-1.0)/Ev;
      P4[i][j] = intc4[i][j]*(intc4[i][j]-0.5)*(intc4[i][j]-1.0)/Ev;
    ...
    ijloop{
        ...
        acount = acount + h*h*( P1[i][j]*int_r*(3.0*nc[i][j]-4.0*c[i][j]+oc[i][j])/(2.0*dt)
               + P2[i][j]*int_r*(3.0*nc2[i][j]-4.0*c2[i][j]+oc2[i][j])/(2.0*dt)+ P3[i][j]*int_r*(3.0*nc3[i][j]-4.0*c3[i][j]+oc3[i][j])/(2.0*dt)
               + P4[i][j]*int_r*(3.0*(1.0-nc[i][j]-nc2[i][j]-nc3[i][j])-4.0*(1.0-c[i][j]-c2[i][j]-c3[i][j])+(1.0-oc[i][j]-oc2[i][j]-oc3[i][j]))/(2.0*dt) );
    ...
    /*  
     ...
     ijloop{
      ...
      EnEO = EnEO + h*h*( 0.25*pow(nc[i][j],2)*pow(nc[i][j]-1.0,2) + 0.25*pow(nc2[i][j],2)*pow(nc2[i][j]-1.0,2) 
         + 0.25*pow(nc3[i][j],2)*pow(nc3[i][j]-1.0,2)+ 0.25*pow(c4[i][j],2)*pow(c4[i][j]-1.0,2)  
         + 0.5*Cahn*( pow(nc[i+1][j]-nc[i][j],2)/(h*h) + pow(nc[i][j+1]-nc[i][j],2)/(h*h) )
         + 0.5*Cahn*( pow(nc2[i+1][j]-nc2[i][j],2)/(h*h) + pow(nc2[i][j+1]-nc2[i][j],2)/(h*h) )
         + 0.5*Cahn*( pow(nc3[i+1][j]-nc3[i][j],2)/(h*h) + pow(nc3[i][j+1]-nc3[i][j],2)/(h*h) )
         + 0.5*Cahn*( pow(c4[i+1][j]-c4[i][j],2)/(h*h) + pow(c4[i][j+1]-c4[i][j],2)/(h*h) )  )
         ;
    ...
    ijloop{
     ...
     ic4[i][j] = 1.0-ic[i][j]-ic2[i][j]-ic3[i][j];
     ...
     ijloop{
      ...
      EnEM = EnEM + h*h*( 
          0.25*Cahn*( pow(nc[i+1][j]-nc[i][j],2)/(h*h) + pow(nc[i][j+1]-nc[i][j],2)/(h*h) )
         + 0.25*Cahn*( pow(nc2[i+1][j]-nc2[i][j],2)/(h*h) + pow(nc2[i][j+1]-nc2[i][j],2)/(h*h) )
         + 0.25*Cahn*( pow(nc3[i+1][j]-nc3[i][j],2)/(h*h) + pow(nc3[i][j+1]-nc3[i][j],2)/(h*h) ) 
          + 0.25*Cahn*( pow(c4[i+1][j]-c4[i][j],2)/(h*h) + pow(c4[i][j+1]-c4[i][j],2)/(h*h) ) 
         ...
         + 0.25*Cahn*( pow(ic3[i+1][j]-ic3[i][j],2)/(h*h) + pow(ic3[i][j+1]-ic3[i][j],2)/(h*h) ) 
          + 0.25*Cahn*( pow(ic4[i+1][j]-ic4[i][j],2)/(h*h) + pow(ic4[i][j+1]-ic4[i][j],2)/(h*h) ) 
         + 0.5*SS*( pow(nc[i][j]-oc[i][j],2)  + pow(nc2[i][j]-oc2[i][j],2) + pow(nc3[i][j]-oc3[i][j],2) + pow(1.0-nc[i][j]-nc2[i][j]-nc3[i][j] - (1.0-oc[i][j]-oc2[i][j]-oc3[i][j]),2)   )  );
    ...
    */   
        ...
        if (it % ns == 0) {
/*             count++;
            print_data(nc,nc2,nc3,c4);
            fprintf(myr,"%16.12f \n",EnEM);
            fprintf(myq,"%16.12f \n",EnEO);
            printf("print out counts %d\n", count); */
        end = clock();
            elapsed = ((float)(end-start))/CLOCKS_PER_SEC;
            fprintf(mycpu,"%16.12f \n", elapsed );
        }
    }
    fclose(mycpu);  
    return 0;
...
void initialization(double **c, double **c2, double **c3,double **c4)
{
...
/*   c[i][j] = 0.25 + 0.05*(2.0*rand()/(float)RAND_MAX - 1.0);
   
   c2[i][j] = 0.25 + 0.05*(2.0*rand()/(float)RAND_MAX - 1.0);

   c3[i][j] = 0.25+0.05*(2.0*rand()/(float)RAND_MAX - 1.0);
...
void source_ini(double **c_old, double **P, double **src_c, double **src_mu)
{
    ...
    ijloop {
        ...
        src_mu[i][j] = (P[i][j] + alp[i][j])*r - SS*c_old[i][j];
    }
...
void relax_ini(double **c_new, double **mu_new, double **su, double **sw, int ilevel, int nxt, int nyt)
{
    ...
    for (iter=1; iter<=c_relax; iter++) {
        ...
        ijloopt {
            ...
            f[0] = su[i][j];
            if (i > 1)   f[0] += M*mu_new[i-1][j]/ht2;
            else         f[0] += M*mu_new[nxt][j]/ht2;
            
            if (i < nxt) f[0] += M*mu_new[i+1][j]/ht2;
            else         f[0] += M*mu_new[1][j]/ht2;
            
           if (j > 1)   f[0] += M*mu_new[i][j-1]/ht2;
            
            
            if (j < nyt) f[0] += M*mu_new[i][j+1]/ht2;
...
void nonL_ini(double **ru, double **rw, double **c_new, double **mu_new, int nxt, int nyt)
{
    ...
    ijloopt {
        ...
        ru[i][j] = c_new[i][j]/dt - M*lap_mu[i][j];
...
void source(double **c_old, double **cc_old, double **P, double **src_c, double **src_mu)
{
    ...
    ijloop {
        ...
        src_mu[i][j] = (P[i][j] + alp[i][j])*int_r - SS*(2.0*c_old[i][j]-cc_old[i][j]);
    }
...
void nonL(double **ru, double **rw, double **c_new, double **mu_new, int nxt, int nyt)
{
    ...
    ijloopt {
        ...
        ru[i][j] = 3.0*c_new[i][j]/(2.0*dt) - M*lap_mu[i][j];
        rw[i][j] = mu_new[i][j] + Cahn*lap_c[i][j] - SS*c_new[i][j];
    }
...
double Ef(double **c, double **c2, double **c3,double **c4, int nxt, int nyt)
{
