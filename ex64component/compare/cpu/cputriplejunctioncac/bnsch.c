...
double pi, xleft, xright, yleft, yright,volume, h, dt, gam, Cahn,M, Ev, oEv, acount, int_r, or, r, nr, EnEO, EnEM,
        **worku, **workv, **workp, **mc, SS, CC,**ct, **sc, **smu, **mu, **H1, **H2, **H3,**H4 ,**alp, **intc1, **intc2,
...
int main()
{
    ...
    /* sprintf(bufferc,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/datac.m");
    sprintf(bufferc2,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/datac2.m");
    sprintf(bufferc3,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/datac3.m");
    sprintf(bufferc4,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/datac4.m");
...
/*      EnEO = 0.0;
     ...
     ijloop{
      ...
      EnEO = EnEO + h*h*( 1/Cahn*0.25*pow(c[i][j],2)*pow(c[i][j]-1.0,2) +1/Cahn* 0.25*pow(c2[i][j],2)*pow(c2[i][j]-1.0,2) 
         + 1/Cahn*0.25*pow(c3[i][j],2)*pow(c3[i][j]-1.0,2) +1/Cahn*0.25*pow(c4[i][j],2)*pow(c4[i][j]-1.0,2)
    ...
    myr = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/Mene.m","w");
    myq = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/triplejunctioncac/dataout/Oene.m","w");
    
    fprintf(myr,"%16.12f \n",EnEO);
    fprintf(myq,"%16.12f \n",EnEO); */
   mycpu = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample44component/compare/cpu/dataoutput/cpucac.m","w");
  ...
  for( it = 1; it <= 1; it++){
    ...
    ijloop{
         ...
         alp[i][j] = -(1.0/Num)*(H1[i][j] + H2[i][j] + H3[i][j] + H4[i][j] );
    ...
    ijloop{
        ...
        acount = acount + h*h*( H1[i][j]*r*(nc[i][j]-c[i][j])/dt
               + H2[i][j]*r*(nc2[i][j]-c2[i][j])/dt+H3[i][j]*r*(nc3[i][j]-c3[i][j])
               + H4[i][j]*r*( (1.0-nc[i][j]-nc2[i][j]-nc3[i][j])-(1.0-c[i][j]-c2[i][j]-c3[i][j]) )/dt );
    ...
    ijloop{
        ...
        acount = acount + h*h*( H1[i][j]*int_r*(3.0*nc[i][j]-4.0*c[i][j]+oc[i][j])/(2.0*dt)
               + H2[i][j]*int_r*(3.0*nc2[i][j]-4.0*c2[i][j]+oc2[i][j])/(2.0*dt)+ H3[i][j]*int_r*(3.0*nc3[i][j]-4.0*c3[i][j]+oc3[i][j])/(2.0*dt)
               + H4[i][j]*int_r*(3.0*(1.0-nc[i][j]-nc2[i][j]-nc3[i][j])-4.0*(1.0-c[i][j]-c2[i][j]-c3[i][j])+(1.0-oc[i][j]-oc2[i][j]-oc3[i][j]))/(2.0*dt) );
     ...
     ijloop{
      ...
      EnEO = EnEO + h*h*(1/Cahn* 0.25*pow(nc[i][j],2)*pow(nc[i][j]-1.0,2) +1/Cahn*0.25*pow(nc2[i][j],2)*pow(nc2[i][j]-1.0,2) 
         +1/Cahn* 0.25*pow(nc3[i][j],2)*pow(nc3[i][j]-1.0,2)+1/Cahn* 0.25*pow(c4[i][j],2)*pow(c4[i][j]-1.0,2)  
     ...
     ijloop{
      ...
      EnEM = EnEM + h*h*( 
         ...
         + 0.25*( pow(nc3[i+1][j]-nc3[i][j],2)/(h*h) + pow(nc3[i][j+1]-nc3[i][j],2)/(h*h) ) 
          + 0.25*( pow(c4[i+1][j]-c4[i][j],2)/(h*h) + pow(c4[i][j+1]-c4[i][j],2)/(h*h) ) 
         ...
         + 0.5*SS/Cahn*( pow(nc[i][j]-oc[i][j],2)  + pow(nc2[i][j]-oc2[i][j],2) + pow(nc3[i][j]-oc3[i][j],2) + pow(1.0-nc[i][j]-nc2[i][j]-nc3[i][j] - (1.0-oc[i][j]-oc2[i][j]-oc3[i][j]),2)   )  );
...
void relax_ini(double **c_new, double **mu_new, double **su, double **sw, int ilevel, int nxt, int nyt)
{
    ...
    for (iter=1; iter<=c_relax; iter++) {
        ...
        ijloopt {
            ...
            /* if (j > 1)   f[1] += c_new[i][j-1]/ht2;
            else         f[1] += c_new[i][nyt]/ht2;
            
            if (j < nyt) f[1] += c_new[i][j+1]/ht2;
            else         f[1] += c_new[i][1]/ht2;
             */
            det = a[0]*a[3] - a[1]*a[2];
