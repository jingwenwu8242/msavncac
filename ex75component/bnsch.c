...
double pi, xleft, xright, yleft, yright,volume, h, dt, gam, Cahn,M, Ev, oEv, acount, int_r, or, r, nr, EnEO, EnEM, CC,
    **worku, **workv, **workp, **mc, SS, **ct, **sc, **smu, **mu, **H1, **H2, **H3, **H4, **H5, **alp, **intc1, **intc2,
    **intc3, **intc4, **intc5, **c5, **oc5, **ic, **ic2, **ic3,**ic4,**ic5;
char bufferc[1000] = {0}, bufferc2[1000] = {0}, bufferc3[1000] = {0}, bufferc4[1000] = {0}, bufferc5[1000] = {0};
int main()
{
    /* extern int nx, ny, n_level, c_relax, count, Num;
    extern double pi, xleft, xright, yleft, yright, h, dt, gam, Cahn, Ev, oEv, acount, int_r, or, r, nr, EnEO, EnEM, CC,
    **worku, **workv, **workp, **mc, SS, **ct, **sc, **smu, **mu, **H1, **H2, **H3, **H4, **H5, **alp, **intc1, **intc2,
    **intc3, **intc4, **intc5, **c5, **oc5, **ic, **ic2, **ic3,**ic4,**ic5;
    extern char bufferc[1000] = {0}, bufferc2[1000] = {0}, bufferc3[1000] = {0}, bufferc4[1000] = {0}, bufferc5[1000] = {0}; */
    int i, j, it, max_it, ns;
    double **c, **nc, **oc, **c2, **nc2, **oc2, **c3, **nc3, **oc3, **c4, **nc4, **oc4;

    FILE *fc, *fc2, *fc3, *fc4, *fc5, *myr, *myq;
    ...
    gam = 4.0 * h / (4.0 * sqrt(2.0) * atanh(0.9)); gam = ε//
    ...
    intc4 = dmatrix(0, nx + 1, 0, ny + 1);
    intc5 = dmatrix(0, nx + 1, 0, ny + 1);

    ic = dmatrix(0, nx+1, 0, ny+1);
    ic2 = dmatrix(0, nx+1, 0, ny+1);
    ic3 = dmatrix(0, nx+1, 0, ny+1);
    ic4 = dmatrix(0, nx+1, 0, ny+1);
    ic5 = dmatrix(0, nx+1, 0, ny+1);
    ...
    oc4 = dmatrix(0, nx + 1, 0, ny + 1); 
    c4 = dmatrix(0, nx + 1, 0, ny + 1);  
    nc4 = dmatrix(0, nx + 1, 0, ny + 1); 

    c5 = dmatrix(0, nx + 1, 0, ny + 1);  
    oc5 = dmatrix(0, nx + 1, 0, ny + 1); 

    H1 = dmatrix(0, nx + 1, 0, ny + 1);  
    H2 = dmatrix(0, nx + 1, 0, ny + 1);  
    H3 = dmatrix(0, nx + 1, 0, ny + 1);  
    H4 = dmatrix(0, nx + 1, 0, ny + 1);  
    H5 = dmatrix(0, nx + 1, 0, ny + 1);  
    ...
    sprintf(bufferc, "/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/datac.m");
    sprintf(bufferc2, "/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/datac2.m");
    sprintf(bufferc3, "/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/datac3.m");
    sprintf(bufferc4, "/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/datac4.m");
    sprintf(bufferc5, "/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/datac5.m");

    fc = fopen(bufferc, "w");
    fc2 = fopen(bufferc2, "w");
    fc3 = fopen(bufferc3, "w");
    fc4 = fopen(bufferc4, "w");
    fc5 = fopen(bufferc5, "w");
    ...
    initialization(c, c2, c3, c4, c5); 

    mat_copy(oc, c, 1, nx, 1, ny);   
    mat_copy(oc2, c2, 1, nx, 1, ny); 
    mat_copy(oc3, c3, 1, nx, 1, ny); 
    mat_copy(oc4, c4, 1, nx, 1, ny); 

    mat_copy(nc, c, 1, nx, 1, ny);   
    mat_copy(nc2, c2, 1, nx, 1, ny); 
    mat_copy(nc3, c3, 1, nx, 1, ny); 
    mat_copy(nc4, c4, 1, nx, 1, ny); 

    print_data(c, c2, c3, c4, c5); 
    ...
    ijloop
    {
        ...
        EnEO = EnEO + h * h * (1/Cahn*0.25 * pow(c[i][j], 2) * pow(c[i][j] - 1.0, 2) 
        + 1/Cahn*0.25 * pow(c2[i][j], 2) * pow(c2[i][j] - 1.0, 2) 
        + 1/Cahn*0.25 * pow(c3[i][j], 2) * pow(c3[i][j] - 1.0, 2) 
        + 1/Cahn*0.25 * pow(c4[i][j], 2) * pow(c4[i][j] - 1.0, 2) 
        + 1/Cahn*0.25 * pow(c5[i][j], 2) * pow(c5[i][j] - 1.0, 2) 
        + 0.5 * (pow(c[i + 1][j] - c[i][j], 2) / (h * h) + pow(c[i][j + 1] - c[i][j], 2) / (h * h)) 
        + 0.5  * (pow(c2[i + 1][j] - c2[i][j], 2) / (h * h) + pow(c2[i][j + 1] - c2[i][j], 2) / (h * h)) 
        + 0.5  * (pow(c3[i + 1][j] - c3[i][j], 2) / (h * h) + pow(c3[i][j + 1] - c3[i][j], 2) / (h * h))
        + 0.5  * (pow(c4[i + 1][j] - c4[i][j], 2) / (h * h) + pow(c4[i][j + 1] - c4[i][j], 2) / (h * h)) 
        + 0.5  * (pow(c5[i + 1][j] - c5[i][j], 2) / (h * h) + pow(c5[i][j + 1] - c5[i][j], 2) / (h * h)));
    }

    myr = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/Mene.m", "w");
    myq = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample55component/dateout/Oene.m", "w");
    ...
    for (it = 1; it <= 1; it++)
    {
        ...
        ijloop
        {
            ...
            H4[i][j] = c4[i][j] * (c4[i][j] - 0.5) * (c4[i][j] - 1.0) / Ev;
            H5[i][j] = c5[i][j] * (c5[i][j] - 0.5) * (c5[i][j] - 1.0) / Ev;
        ...
        ijloop{
            ...
            alp[i][j] = -(1.0 / Num) * (H1[i][j] + H2[i][j] + H3[i][j] + H4[i][j] + H5[i][j]);
        ...
        ijloop { c5[i][j] = 1.0 - nc[i][j] - nc2[i][j] - nc3[i][j] - nc4[i][j]; }
        ...
        ijloop
        {
            ...
            + H3[i][j] * r * (nc3[i][j] - c3[i][j]) 
            + H4[i][j] * r * (nc4[i][j] - c4[i][j])
             + H5[i][j] * r * ((1.0 - nc[i][j] - nc2[i][j] - nc3[i][j] - nc4[i][j]) - (1.0 - c[i][j] - c2[i][j] - c3[i][j] - c4[i][j])) / dt);
        }
        ...
        mat_copy(c3, nc3, 1, nx, 1, ny); 
        mat_copy(c4, nc4, 1, nx, 1, ny); 
    }
    ...
    for (it = 2; it <= max_it; it++)
    {
        ...
        ijloop
        {
            ...
            intc3[i][j] = 2.0 * c3[i][j] - oc3[i][j];
            intc4[i][j] = 2.0 * c4[i][j] - oc4[i][j];
            intc5[i][j] = 1.0 - intc1[i][j] - intc2[i][j] - intc3[i][j] - intc4[i][j];

        } 

        Ev = Ef(intc1, intc2, intc3, intc4, intc5, nx, ny); 
        ...
        ijloop
        {
            ...
            H4[i][j] = intc4[i][j] * (intc4[i][j] - 0.5) * (intc4[i][j] - 1.0) / Ev;
            H5[i][j] = intc5[i][j] * (intc5[i][j] - 0.5) * (intc5[i][j] - 1.0) / Ev;
      ...
      ijloop{
        ...
        ijloop
        {
            ...
            + H3[i][j] * int_r * (3.0 * nc3[i][j] - 4.0 * c3[i][j] + oc3[i][j]) / (2.0 * dt) 
            + H4[i][j] * int_r * (3.0 * nc4[i][j] - 4.0 * c4[i][j] + oc4[i][j]) / (2.0 * dt) 
            + H5[i][j] * int_r * (3.0 * (1.0 - nc[i][j] - nc2[i][j] - nc3[i][j] - nc4[i][j]) - 4.0 * (1.0 - c[i][j] - c2[i][j] - c3[i][j] - c4[i][j]) + (1.0 - oc[i][j] - oc2[i][j] - oc3[i][j] - oc4[i][j])) / (2.0 * dt));
        }
        ...
        ijloop
        {
            ...
            EnEO = EnEO + h * h * (1/Cahn*0.25 * pow(nc[i][j], 2) * pow(nc[i][j] - 1.0, 2)
             + 1/Cahn*0.25 * pow(nc2[i][j], 2) * pow(nc2[i][j] - 1.0, 2)
              + 1/Cahn*0.25 * pow(nc3[i][j], 2) * pow(nc3[i][j] - 1.0, 2) 
              + 1/Cahn*0.25 * pow(nc4[i][j], 2) * pow(nc4[i][j] - 1.0, 2) 
              + 1/Cahn*0.25 * pow(c5[i][j], 2) * pow(c5[i][j] - 1.0, 2)
               + 0.5  * (pow(nc[i + 1][j] - nc[i][j], 2) / (h * h) + pow(nc[i][j + 1] - nc[i][j], 2) / (h * h)) 
               + 0.5  * (pow(nc2[i + 1][j] - nc2[i][j], 2) / (h * h) + pow(nc2[i][j + 1] - nc2[i][j], 2) / (h * h)) 
               + 0.5  * (pow(nc3[i + 1][j] - nc3[i][j], 2) / (h * h) + pow(nc3[i][j + 1] - nc3[i][j], 2) / (h * h)) 
               + 0.5  * (pow(nc4[i + 1][j] - nc4[i][j], 2) / (h * h) + pow(nc4[i][j + 1] - nc4[i][j], 2) / (h * h)) 
               + 0.5  * (pow(c5[i + 1][j] - c5[i][j], 2) / (h * h) + pow(c5[i][j + 1] - c5[i][j], 2) / (h * h)));
        }
     ...
     ic[i][j] = 2.0*nc[i][j] - oc[i][j];
     ic2[i][j] = 2.0*nc2[i][j] - oc2[i][j];
     ic3[i][j] = 2.0*nc3[i][j] - oc3[i][j];
     ic4[i][j] = 2.0*nc4[i][j] - oc4[i][j];
     ic5[i][j] = 1.0-ic[i][j]-ic2[i][j]-ic3[i][j]-ic4[i][j];
        ...
        ijloop
        {
            ...
            + 0.25  * (pow(nc3[i + 1][j] - nc3[i][j], 2) / (h * h) + pow(nc3[i][j + 1] - nc3[i][j], 2) / (h * h)) 
            + 0.25  * (pow(nc4[i + 1][j] - nc4[i][j], 2) / (h * h) + pow(nc4[i][j + 1] - nc4[i][j], 2) / (h * h)) 
            + 0.25  * (pow(c5[i + 1][j] - c5[i][j], 2) / (h * h) + pow(c5[i][j + 1] - c5[i][j], 2) / (h * h))
         ...
         + 0.25*( pow(ic4[i+1][j]-ic4[i][j],2)/(h*h) + pow(ic4[i][j+1]-ic4[i][j],2)/(h*h) ) 
         + 0.25*( pow(ic5[i+1][j]-ic5[i][j],2)/(h*h) + pow(ic5[i][j+1]-ic5[i][j],2)/(h*h) ) 

         + 0.5*SS/Cahn*( pow(nc[i][j]-oc[i][j],2)  + pow(nc2[i][j]-oc2[i][j],2) + pow(nc3[i][j]-oc3[i][j],2) + pow(nc4[i][j]-oc4[i][j],2)  
         + pow(1.0-nc[i][j]-nc2[i][j]-nc3[i][j]-nc4[i][j] - (1.0-oc[i][j]-oc2[i][j]-oc3[i][j]-oc4[i][j]),2)   )  );
...
void initialization(double **c, double **c2, double **c3, double **c4, double **c5)
{
    ...
    ijloop
    {
        ...
        /*  c[i][j] = 0.2 + 0.01*(2.0*rand()/(float)RAND_MAX - 1.0);
   
   c2[i][j] = 0.2 + 0.01*(2.0*rand()/(float)RAND_MAX - 1.0);

   c3[i][j] = 0.2+0.01*(2.0*rand()/(float)RAND_MAX - 1.0);

   c4[i][j] = 0.2+0.01*(2.0*rand()/(float)RAND_MAX - 1.0);



   c5[i][j] = 1.0 - c[i][j] - c2[i][j]-c3[i][j]-c4[i][j]; */
        
       /* c[i][j] =c2[i][j] =c3[i][j] =c4[i][j]= 0;
        if (x >= 0.25 && x <= 0.5 && y >= 0.25 && y <= 0.58)
            c[i][j] = 1;
        if (x > 0.5 && x <= 0.75 && y >= 0.25 && y <= 0.58)
            c2[i][j] = 1;
        if (x >= 0.25 && x <= 0.5 && y > 0.58 && y <= 0.75)
            c3[i][j] = 1;
        if(x >= 0.5 && x <= 0.75 && y > 0.58 && y <= 0.75)
            c4[i][j]=1;
        c5[i][j]=1.0 - c[i][j] - c2[i][j]-c3[i][j]-c4[i][j];  */
         
       if ((rand() / (double)RAND_MAX )< 0.2){
            c[i][j] = 1;
        c2[i][j] =0; c3[i][j] = 0;c4[i][j] = 0;c5[i][j] = 0;
        }
        else if ((rand() / (double )RAND_MAX) < 0.2){
            c2[i][j] = 1;
        c[i][j] =0; c3[i][j] =0; c4[i][j] =0; c5[i][j] = 0;
        }
        else if ((rand() / (double)RAND_MAX) < 0.4){
            c3[i][j] = 1;
        c[i][j] = 0;c2[i][j] =0; c4[i][j] =0; c5[i][j] = 0;
        }
        else if ((rand() / (double)RAND_MAX )< 0.6){
            c4[i][j] = 1;
        c[i][j] =0; c2[i][j] = 0;c3[i][j] = 0;c5[i][j] = 0;
        }
        else {c5[i][j] = 1;
        c[i][j] =0; c2[i][j] =0; c3[i][j] = 0;c4[i][j] = 0;
        } 
    } 
...
{
    extern int c_relax;
    extern double xright, dt, Cahn,M, SS;
...
{
    extern double xright, dt, SS, M,Cahn;
...
{
    extern int nx, ny;
    extern double dt, h, int_r, gam, SS, **alp;
...
double Ef(double **c, double **c2, double **c3, double **c4, double **c5, int nxt, int nyt)
{
    ...
    ijloopt { r = r + h * h * (0.25 * pow(c[i][j], 2) * pow(c[i][j] - 1.0, 2) + 0.25 * pow(c2[i][j], 2) * pow(c2[i][j] - 1.0, 2) + 0.25 * pow(c3[i][j], 2) * pow(c3[i][j] - 1.0, 2) + 0.25 * pow(c4[i][j], 2) * pow(c4[i][j] - 1.0, 2) + 0.25 * pow(c5[i][j], 2) * pow(c5[i][j] - 1.0, 2)); }
...
void print_data(double **c, double **c2, double **c3, double **c4, double **c5)
{
    extern char bufferc[1000], bufferc2[1000], bufferc3[1000], bufferc4[1000], bufferc5[1000];
    int i, j;
    FILE *fc, *fc2, *fc3, *fc4, *fc5;

    fc = fopen(bufferc, "a");
    fc2 = fopen(bufferc2, "a");
    fc3 = fopen(bufferc3, "a");
    fc4 = fopen(bufferc4, "a");
    fc5 = fopen(bufferc5, "a");
