/************************************************************************
all is right and  run  successfully
   Binary NSCH
...
float pi, xleft, xright, yleft, yright,zleft,zright, h, dt, gam, Cahn,  Ev, oEv, acount, int_r, or, r, nr, EnEO, EnEM,
        ***worku, ***workv, ***workp, ***workw, ***mc, SS, M,CC,***ct, ***sc, ***smu, ***mu, ***P1, ***P2, ***P3,***P4 ,***alp, ***intc1, ***intc2,
...
int main()
{
    ...
    extern float pi, xleft, xright, yleft, yright, h, dt, gam, Cahn, Ev,  oEv, acount, int_r, or, r, nr, EnEO, EnEM,
                  ***worku, ***workv, ***workp, ***mc, SS, ***ct, ***sc, ***smu, ***mu, ***P1, ***P2, ***P3, ***alp, ***intc1, ***intc2,
                  ***intc3,***intc4, ***c4, ***oc4;

    extern char  bufferc[1000], bufferc2[1000], bufferc3[1000],bufferc4[1000]; */
    int i,j,k, it, max_it, ns;
    float ***c, ***nc, ***oc, ***c2, ***nc2, ***oc2,***c3, ***nc3, ***oc3,***ic, ***ic2, ***ic3, ***ic4;
    ...
    P1 = cube(0, nx+1, 0, ny+1, 0, nz+1);
    P2 = cube(0, nx+1, 0, ny+1, 0, nz+1);
    P3 = cube(0, nx+1, 0, ny+1, 0, nz+1);
    P4 = cube(0, nx+1, 0, ny+1, 0, nz+1);
    ...
    sprintf(bufferc,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac.m");
    sprintf(bufferc2,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac2.m");
    sprintf(bufferc3,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac3.m");
    sprintf(bufferc4,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac4.m");
    ...
    augmenc(c4, nx, ny,nz);
     ...
     ijkloop{
      ...
      EnEO = EnEO + h*h*( 0.25*pow(c[i][j][k],2)*pow(c[i][j][k]-1.0,2) + 0.25*pow(c2[i][j][k],2)*pow(c2[i][j][k]-1.0,2) 
         + 0.25*pow(c3[i][j][k],2)*pow(c3[i][j][k]-1.0,2) +0.25*pow(c4[i][j][k],2)*pow(c4[i][j][k]-1.0,2)
         + 0.5*Cahn*( pow(c[i+1][j][k]-c[i][j][k],2)/(h*h) + pow(c[i][j+1][k]-c[i][j][k],2)/(h*h)+ pow(c[i][j][k+1]-c[i][j][k],2)/(h*h) )
         + 0.5*Cahn*( pow(c2[i+1][j][k]-c2[i][j][k],2)/(h*h) + pow(c2[i][j+1][k]-c2[i][j][k],2)/(h*h)+ pow(c2[i][j][k+1]-c[i][j][k],2)/(h*h) )
         + 0.5*Cahn*( pow(c3[i+1][j][k]-c3[i][j][k],2)/(h*h) + pow(c3[i][j+1][k]-c3[i][j][k],2)/(h*h)+ pow(c3[i][j][k+1]-c[i][j][k],2)/(h*h) )  
         + 0.5*Cahn*( pow(c4[i+1][j][k]-c4[i][j][k],2)/(h*h) + pow(c4[i][j+1][k]-c4[i][j][k],2)/(h*h)+ pow(c4[i][j][k+1]-c[i][j][k],2)/(h*h) )  );
         
     }
       
    myr = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/Mene.m","w");
    myq = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/Oene.m","w");
  ...
  for( it = 1; it <= 1; it++){
    ...
    ijkloop{
      ...
      P1[i][j][k] = c[i][j][k]*(c[i][j][k]-0.5)*(c[i][j][k]-1.0)/Ev;
      P2[i][j][k] = c2[i][j][k]*(c2[i][j][k]-0.5)*(c2[i][j][k]-1.0)/Ev;
      P3[i][j][k] = c3[i][j][k]*(c3[i][j][k]-0.5)*(c3[i][j][k]-1.0)/Ev;
      P4[i][j][k] = c4[i][j][k]*(c4[i][j][k]-0.5)*(c4[i][j][k]-1.0)/Ev;

      alp[i][j][k] = -(1.0/Num)*(P1[i][j][k] + P2[i][j][k] + P3[i][j][k] + P4[i][j][k] );
    ...
    ijkloop{
        ...
        acount = acount + h*h*( P1[i][j][k]*r*(nc[i][j][k]-c[i][j][k])/dt
               + P2[i][j][k]*r*(nc2[i][j][k]-c2[i][j][k])/dt+P3[i][j][k]*r*(nc3[i][j][k]-c3[i][j][k])
               + P4[i][j][k]*r*( (1.0-nc[i][j][k]-nc2[i][j][k]-nc3[i][j][k])-(1.0-c[i][j][k]-c2[i][j][k]-c3[i][j][k]) )/dt );
    ...
    ijkloop{
      ...
      P1[i][j][k] = intc1[i][j][k]*(intc1[i][j][k]-0.5)*(intc1[i][j][k]-1.0)/Ev;
      P2[i][j][k] = intc2[i][j][k]*(intc2[i][j][k]-0.5)*(intc2[i][j][k]-1.0)/Ev;
      P3[i][j][k] = intc3[i][j][k]*(intc3[i][j][k]-0.5)*(intc3[i][j][k]-1.0)/Ev;
      P4[i][j][k] = intc4[i][j][k]*(intc4[i][j][k]-0.5)*(intc4[i][j][k]-1.0)/Ev;
    ...
    ijkloop{
        ...
        acount = acount + h*h*( P1[i][j][k]*int_r*(3.0*nc[i][j][k]-4.0*c[i][j][k]+oc[i][j][k])/(2.0*dt)
               + P2[i][j][k]*int_r*(3.0*nc2[i][j][k]-4.0*c2[i][j][k]+oc2[i][j][k])/(2.0*dt)+ P3[i][j][k]*int_r*(3.0*nc3[i][j][k]-4.0*c3[i][j][k]+oc3[i][j][k])/(2.0*dt)
               + P4[i][j][k]*int_r*(3.0*(1.0-nc[i][j][k]-nc2[i][j][k]-nc3[i][j][k])-4.0*(1.0-c[i][j][k]-c2[i][j][k]-c3[i][j][k])+(1.0-oc[i][j][k]-oc2[i][j][k]-oc3[i][j][k]))/(2.0*dt) );

     }


        nr = ( 4.0*r - or + 2.0*dt*acount )/3.0;
     ...
     ijkloop{
      ...
      EnEO = EnEO + h*h*( 0.25*pow(nc[i][j][k],2)*pow(nc[i][j][k]-1.0,2) + 0.25*pow(nc2[i][j][k],2)*pow(nc2[i][j][k]-1.0,2) 
         + 0.25*pow(nc3[i][j][k],2)*pow(nc3[i][j][k]-1.0,2)+ 0.25*pow(c4[i][j][k],2)*pow(c4[i][j][k]-1.0,2)  
         + 0.5*Cahn*( pow(nc[i+1][j][k]-nc[i][j][k],2)/(h*h) + pow(nc[i][j+1][k]-nc[i][j][k],2)/(h*h)+ pow(nc[i][j][k+1]-nc[i][j][k],2)/(h*h) )
         + 0.5*Cahn*( pow(nc2[i+1][j][k]-nc2[i][j][k],2)/(h*h) + pow(nc2[i][j+1][k]-nc2[i][j][k],2)/(h*h) + pow(nc2[i][j][k+1]-nc2[i][j][k],2)/(h*h))
         + 0.5*Cahn*( pow(nc3[i+1][j][k]-nc3[i][j][k],2)/(h*h) + pow(nc3[i][j+1][k]-nc3[i][j][k],2)/(h*h)+ pow(nc3[i][j][k+1]-nc3[i][j][k],2)/(h*h) )
         + 0.5*Cahn*( pow(c4[i+1][j][k]-c4[i][j][k],2)/(h*h) + pow(c4[i][j+1][k]-c4[i][j][k],2)/(h*h) ) + pow(c4[i][j][k+1]-c4[i][j][k],2)/(h*h) )
         ;
    ...
    ijkloop{
     ...
     ic3[i][j][k] = 2.0*nc3[i][j][k] - oc3[i][j][k];
     ...
     ijkloop{
      ...
      EnEM = EnEM + h*h*( 
          0.25*Cahn*( pow(nc[i+1][j][k]-nc[i][j][k],2)/(h*h) + pow(nc[i][j+1][k]-nc[i][j][k],2)/(h*h)+ pow(nc[i][j][k+1]-nc[i][j][k],2)/(h*h) )
         + 0.25*Cahn*( pow(nc2[i+1][j][k]-nc2[i][j][k],2)/(h*h) + pow(nc2[i][j+1][k]-nc2[i][j][k],2)/(h*h)+ pow(nc2[i][j][k+1]-nc2[i][j][k],2)/(h*h) )
         + 0.25*Cahn*( pow(nc3[i+1][j][k]-nc3[i][j][k],2)/(h*h) + pow(nc3[i][j+1][k]-nc3[i][j][k],2)/(h*h)+ pow(nc3[i][j][k+1]-nc3[i][j][k],2)/(h*h) ) 
          + 0.25*Cahn*( pow(c4[i+1][j][k]-c4[i][j][k],2)/(h*h) + pow(c4[i][j+1][k]-c4[i][j][k],2)/(h*h)+ pow(c4[i][j][k+1]-c4[i][j][k],2)/(h*h) ) 
         
         + 0.25*Cahn*( pow(ic[i+1][j][k]-ic[i][j][k],2)/(h*h) + pow(ic[i][j+1][k]-ic[i][j][k],2)/(h*h) + pow(ic[i][j][k+1]-ic[i][j][k],2)/(h*h) )  
         + 0.25*Cahn*( pow(ic2[i+1][j][k]-ic2[i][j][k],2)/(h*h) + pow(ic2[i][j+1][k]-ic2[i][j][k],2)/(h*h) + pow(ic2[i][j][k+1]-ic2[i][j][k],2)/(h*h))  
         + 0.25*Cahn*( pow(ic3[i+1][j][k]-ic3[i][j][k],2)/(h*h) + pow(ic3[i][j+1][k]-ic3[i][j][k],2)/(h*h) + pow(ic3[i][j][k+1]-ic3[i][j][k],2)/(h*h)) 
          + 0.25*Cahn*( pow(ic4[i+1][j][k]-ic4[i][j][k],2)/(h*h) + pow(ic4[i][j+1][k]-ic4[i][j][k],2)/(h*h) + pow(ic4[i][j][k+1]-ic4[i][j][k],2)/(h*h)) 
         + 0.5*SS*( pow(nc[i][j][k]-oc[i][j][k],2)  + pow(nc2[i][j][k]-oc2[i][j][k],2) + pow(nc3[i][j][k]-oc3[i][j][k],2) + pow(1.0-nc[i][j][k]-nc2[i][j][k]-nc3[i][j][k] - (1.0-oc[i][j][k]-oc2[i][j][k]-oc3[i][j][k]),2)   )  );
         
     }
     
     
     EnEM = EnEM + 0.5*(3.0*r-or)-CC;
...
void initialization(float ***c, float ***c2, float ***c3, float ***c4)
{
   ...
   c[i][j][k] = 0.25 + 0.01*(2.0*rand()/(float)RAND_MAX - 1.0);  
   c2[i][j][k] = 0.25 + 0.01*(2.0*rand()/(float)RAND_MAX - 1.0);
   c3[i][j][k] = 0.25 + 0.01*(2.0*rand()/(float)RAND_MAX - 1.0);
...
void source_ini(float ***c_old, float ***P, float ***src_c, float ***src_mu)
{
    ...
    ijkloop {
        ...
        src_mu[i][j][k] = (P[i][j][k] + alp[i][j][k])*r - SS*c_old[i][j][k];
    }
...
void relax_ini(float ***c_new, float ***mu_new, float ***su, float ***sw, int ilevel, int nxt, int nyt,int nzt)
{
    ...
    for (iter=1; iter<=c_relax; iter++) {
        ...
        ijkloopt {
            ...
            if (k > 1)   a[1] += M*1/ht2; 
            if (k < nzt) a[1] += M*1/ht2;
                   
            a[2] = -Cahn*4.0/ht2 - SS;

            if (k > 1)   a[2] -= Cahn/ht2; 
            if (k < nzt) a[2] -= Cahn/ht2;
            ...
            f[0] = su[i][j][k];
            if (i > 1)   f[0] += M*mu_new[i-1][j][k]/ht2;
            else         f[0] += M*mu_new[nxt][j][k]/ht2;
            
            if (i < nxt) f[0] +=M* mu_new[i+1][j][k]/ht2;
            else         f[0] +=M* mu_new[1][j][k]/ht2;
            
           if (j > 1)   f[0] += M*mu_new[i][j-1][k]/ht2;
            else         f[0] += M*mu_new[i][nyt][k]/ht2;
            
            if (j < nyt) f[0] += M*mu_new[i][j+1][k]/ht2;
            else         f[0] += M*mu_new[i][1][k]/ht2;

            if (k > 1)   f[0] += M*mu_new[i][j][k-1]/ht2;   
            if (k < nzt) f[0] +=M* mu_new[i][j][k+1]/ht2;

            
            f[1] = sw[i][j][k];
            if (i > 1)   f[1] += -Cahn*c_new[i-1][j][k]/ht2;
            else         f[1] += -Cahn*c_new[nxt][j][k]/ht2;
            
            if (i < nxt) f[1] += -Cahn*c_new[i+1][j][k]/ht2;
            else         f[1] += -Cahn*c_new[1][j][k]/ht2;
            
            if (j > 1)   f[1] += -Cahn*c_new[i][j-1][k]/ht2;
            else         f[1] += -Cahn*c_new[i][nyt][k]/ht2;
            
            if (j < nyt) f[1] += -Cahn*c_new[i][j+1][k]/ht2;
            else         f[1] += -Cahn*c_new[i][1][k]/ht2;

            if (k > 1)   f[1] += -Cahn*c_new[i][j][k-1]/ht2;
            
            if (k < nzt) f[1] += -Cahn*c_new[i][j][k+1]/ht2;
...
void nonL_ini(float ***ru, float ***rw, float ***c_new, float ***mu_new, int nxt, int nyt,int nzt)
{
    ...
    ijkloopt {
        ...
        ru[i][j][k] = c_new[i][j][k]/dt - M*lap_mu[i][j][k];
        rw[i][j][k] = mu_new[i][j][k] + Cahn*lap_c[i][j][k] - SS*c_new[i][j][k];
    }
...
{
    extern int nx,ny,nz;
    extern float dt, h, int_r, gam, SS, ***alp;
    ...
    ijkloop {
        ...
        src_mu[i][j][k] = (P[i][j][k] + alp[i][j][k])*int_r - SS*(2.0*c_old[i][j][k]-cc_old[i][j][k]);
    }
...
void nonL(float ***ru, float ***rw, float ***c_new, float ***mu_new, int nxt, int nyt,int nzt)
{
    ...
    ijkloopt {
        ...
        ru[i][j][k] = 3.0*c_new[i][j][k]/(2.0*dt) - M*lap_mu[i][j][k];
        rw[i][j][k] = mu_new[i][j][k] + Cahn*lap_c[i][j][k] - SS*c_new[i][j][k];
    }
...
float Ef(float ***c, float ***c2, float ***c3,float ***c4, int nxt, int nyt, int nzt)
{
   ...
   ijkloopt{ r = r + h*h*( 0.25*pow(c[i][j][k],2)*pow(c[i][j][k]-1.0,2) + 0.25*pow(c2[i][j][k],2)*pow(c2[i][j][k]-1.0,2) 
         + 0.25*pow(c3[i][j][k],2)*pow(c3[i][j][k]-1.0,2) + 0.25*pow(c4[i][j][k],2)*pow(c4[i][j][k]-1.0,2) );  }
...
void print_data(float ***c1,float ***c2,float ***c3,float ***c4,int kk)
{
   ...
   extern char bufferc[2000],bufferc2[2000],bufferc3[2000],bufferc3[2000],bufferc4[2000];
   FILE *fc,*fc2,*fc3,*fc4;
   int i,j,k;

    sprintf(bufferc,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac1%d.m",kk);
    sprintf(bufferc2,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac2%d.m",kk);
    sprintf(bufferc3,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac3%d.m",kk);
    sprintf(bufferc4,"/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/example9cac3D/compare3Dtriplejunction/triplejunctionch/dataout/datac4%d.m",kk);
