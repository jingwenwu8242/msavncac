/************************************************
all is right and  run  successfully from y
...
double pi, xleft, xright, yleft, yright, h, dt, gam, Cahn,M,  Ev, oEv, acount, int_r, orc, r, nr, EnEO, EnEM,
...
int main()
{
    ...
    extern double pi, xleft, xright, yleft, yright, h, dt, gam, Cahn, Ev,  oEv, acount, int_r, orc, r, nr, EnEO, EnEM,
                  **worku, **workv, **workp, **mc, SS, **ct, **sc, **smu, **mu, **P1, **P2, **P3, **alp, **intc1, **intc2,
                  **intc3, **c3, **oc3;

    extern char  bufferc[1000], bufferc2[1000], bufferc3[1000];
    int i,j, it, max_it, ns;
    double **c, **nc, **oc, **c2, **nc2, **oc2,elapsed;
    clock_t start,end;
    
    FILE  *fc, *fc2, *fc3, *myr, *myq,*mycpu;
    ...
    /* sprintf(bufferc,"/Users/wujingwen/research/fluid_mechanics/SAV+multi-component incompressible fluid system/msav+nc/data_output/3componentcompare/datac.m");
    sprintf(bufferc2,"/Users/wujingwen/research/fluid_mechanics/SAV+multi-component incompressible fluid system/msav+nc/data_output/3componentcompare/datac2.m");
    sprintf(bufferc3,"/Users/wujingwen/research/fluid_mechanics/SAV+multi-component incompressible fluid system/msav+nc/data_output/3componentcompare/datac3.m");
   ...
   save as row//
    ...
    //myr = fopen("/Users/wujingwen/research/fluid_mechanics/SAV+multi-component incompressible fluid system/msav+nc/data_output/3componentcompare/Mene.m","w");
    //myq = fopen("/Users/wujingwen/research/fluid_mechanics/SAV+multi-component incompressible fluid system/msav+nc/data_output/3componentcompare/Oene.m","w");
    mycpu = fopen("/Users/wujingwen/research/fluid_mechanics/msavncac/msav+ncac/cacexample3compareCH/cpu/dataoutcpu/cpuch.m","w");
  ...
  for( it = 1; it <= 1; it++){
    ...
    ijloop{
      ...
      alp[i][j] = -(1.0/Num)*(P1[i][j] + P2[i][j] + P3[i][j] );
    ...
    ijloop{
        ...
        acount = acount + h*h*( P1[i][j]*r*(nc[i][j]-c[i][j])/dt
               + P2[i][j]*r*(nc2[i][j]-c2[i][j])/dt
               + P3[i][j]*r*( (1.0-nc[i][j]-nc2[i][j])-(1.0-c[i][j]-c2[i][j]) )/dt );
    ...
    ijloop{
        ...
        acount = acount + h*h*( P1[i][j]*int_r*(3.0*nc[i][j]-4.0*c[i][j]+oc[i][j])/(2.0*dt)
               + P2[i][j]*int_r*(3.0*nc2[i][j]-4.0*c2[i][j]+oc2[i][j])/(2.0*dt)
               + P3[i][j]*int_r*(3.0*(1.0-nc[i][j]-nc2[i][j])-4.0*(1.0-c[i][j]-c2[i][j])+(1.0-oc[i][j]-oc2[i][j]))/(2.0*dt) );
